...
// this version tries to scroll smoothly without requiring redundant data to be sent.
...
void scroll(int len) {
  ...
  while (*m and (right_here > CHARACTERS)) {
//  while (*m) {
    //step++;
...
int getCustomData(int len) {
  ...
  if (len !=0 ){
    len = Serial.readBytesUntil('\r', str+CHARACTERS, STRINGBUFFER_LEN);
  }
  else {
    len = Serial.readBytesUntil('\r', str, STRINGBUFFER_LEN);
  }
  // Null terminate
  str[len+CHARACTERS] = 0x00;
  return len;
}
